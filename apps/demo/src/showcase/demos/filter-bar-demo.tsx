...
<ActiveFilters fields={fields} filters={filters} onChange={setFilters} />`}
      >
        <div className="w-full space-y-3">
          <p className="text-sm text-muted-foreground">
            Click the filter icon to select a field. Active filters appear as segmented chips below with operator and
            value controls.
          </p>
          ...
          <ActiveFilters fields={fields} filters={filters} onChange={setFilters} />
          {filters.length > 0 && (
            <pre className="mt-3 rounded-md bg-muted p-3 text-xs">{JSON.stringify(filters, null, 2)}</pre>
          )}
        </div>
      </DemoSection>
      ...
      <DemoSection title="With Pre-set Filters">
        <div className="w-full space-y-3">
          <p className="text-sm text-muted-foreground">
            Filters can be initialized with values including operators. Matches the reference design with date range and
            boolean filters.
          </p>
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
            ...
            <FilterButton fields={fields} activeKeys={presetActiveKeys} onAdd={handlePresetAdd} />
          </div>
          <ActiveFilters fields={fields} filters={presetFilters} onChange={setPresetFilters} />
          <pre className="mt-3 rounded-md bg-muted p-3 text-xs">{JSON.stringify(presetFilters, null, 2)}</pre>
        </div>
      </DemoSection>
    </>
  );
