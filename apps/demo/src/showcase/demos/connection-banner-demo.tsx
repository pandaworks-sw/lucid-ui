...
/>`}
      >
        <div className="space-y-4">
          <Button size="sm" variant={isOnline ? 'destructive' : 'default'} onClick={() => setIsOnline(!isOnline)}>
            {isOnline ? 'Simulate Offline' : 'Go Online'}
          </Button>

          <div className="relative h-20 overflow-hidden rounded-lg border bg-muted/30">
            <ConnectionBanner isOnline={isOnline} onRetry={handleRetry} className="relative" />
            {isOnline && (
