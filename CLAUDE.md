...
### Recommended: route to `components/pandaworks-ui/`

Before consumers install any component, recommend they set the `ui` alias in their project's `components.json` to a dedicated folder so pandaworks components don't share space with their own shadcn primitives:

```json
{
  "aliases": {
    "ui": "@/components/pandaworks-ui"
  }
}
```

shadcn rewrites the `@/components/ui/*` imports inside registry sources during install based on this alias, so internal cross-component references stay valid. The benefit: re-running `_all` only overwrites the `pandaworks-ui` folder — any consumer-local forks of standard shadcn components under `components/ui` survive untouched.
...
Use this for first-time install of every component, or as a "I don't know what changed upstream, just sync me to latest" upgrade path. Drop `--overwrite` to be prompted per file. The `_all.ts` marker file it places at `@/components/pandaworks-ui/_all/_all.ts` has no runtime behavior — delete it after install if you want.
