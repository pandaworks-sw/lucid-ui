...
- Recommended install location for consumers is now `components/pandaworks-ui/` (consumer sets `aliases.ui` to `@/components/pandaworks-ui` in their `components.json`). Keeps pandaworks components in a clearly-owned folder, prevents collisions with the consumer's own shadcn primitives under `components/ui/`, and makes `_all` re-installs surgical (only the pandaworks folder gets rewritten). shadcn rewrites internal `@/components/ui/*` imports during install based on the alias, so cross-component references stay valid. Documented in CLAUDE.md "Consuming" and `public/llms.txt` "Installation"
