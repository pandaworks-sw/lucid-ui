...
type SplitButtonVariant = NonNullable<VariantProps<typeof splitButtonVariants>['variant']>;
  ...
  return (
    <SplitButtonContext.Provider value={ctx}>
      <DropdownMenuPrimitive.Root>
        <div className={cn('inline-flex items-stretch', className)}>{children}</div>
      </DropdownMenuPrimitive.Root>
...
interface SplitButtonActionProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  children: ReactNode;
}
...
const SplitButtonAction = forwardRef<HTMLButtonElement, SplitButtonActionProps>(
  ({ className, children, disabled: disabledProp, ...props }, ref) => {
    const { variant, size, disabled, loading } = useContext(SplitButtonContext);
